...
class BinaryTree{
	...
	public:
		...
		bool checkCompleteBinaryTree(){
			...
			{
				arrayofBTNodes[i].getNodeId().assignLevelNumbers();
				if(!isLeafNode(arrayofBTNodes[i].getNodeId()))
				{
					if(arrayofBTNodes[i].getNodeId().getLeftChildID()==-1||arrayofBTNodes[i].getNodeId().getRightChildID()==-1)
						return false;
						
				}
			else {
				     int b= arrayofBTNodes[i].getNodeId().getLevelNum()
				     if(b!=a)
				     return false;
			}
			}
