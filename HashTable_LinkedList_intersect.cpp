...
int main(){
	...
	List firstList;
	cout << "Elements generated for the first list: ";
	...
	List secondList;
	cout << "Elements generated for the second list: ";
	for (int index = 0; index < numElements; index++){
		int value = rand() % maxValue;
	...
	// elements are to be included only once in the union
	for (int index = 0; index < numElements; index++){
		...
		if(!IntersectionList.containsElement(value))
		{
			IntersectionList.insert(value);
		}
	}
	
	// unionList print - only unique elements
	cout << "Elements in the Intersection list: ";
	IntersectionList.IterativePrint();
