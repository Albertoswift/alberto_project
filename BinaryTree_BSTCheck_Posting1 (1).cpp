...
class BinaryTree{
	...
	public:
		...
		bool checkBST(){
			   ...
			   {
			   		int a=arrayofBTNodes[i].getNodeId();
			   		int b=arrayofBTNodes[i].getNodeId().getLeftChildID();
			   		int c=arrayofBTNodes[i].getNodeId().getRightChildID();
			   		if(!isLeafNode(a))
			   	{
					   	if(a.getData()<b.getData()||a.getData>c.getData)
			   			return false
				}
			   }
